...
describe("sign-in account chooser", () => {
  test("parses remembered accounts defensively and sorts newest first", () => {
    const accounts = parseRememberedSignInAccounts(JSON.stringify([
      { id: "old", email: "OLD@EXAMPLE.COM", name: "Old", lastSeenAt: "2026-01-01T00:00:00.000Z" },
      { id: "new", email: "new@example.com", name: "New", lastSeenAt: "2026-02-01T00:00:00.000Z" },
  ...
  test("remembers the newest account once and caps the list", () => {
    const current = Array.from({ length: RECENT_SIGN_IN_ACCOUNTS_MAX }, (_, index) => ({
      id: `u${index}`,
  ...
  test("loads a localized generic sign-in failure message", async () => {
    ...
    expect(messages.signInFailed).not.toBe(messages.continueWithGoogle);
  });
});
